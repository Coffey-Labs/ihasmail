...
test("the app shell is gzipped when the client asks for it", async () => {
  const app = createApp();
...
test("a client that does not ask for gzip does not get it", async () => {
  const app = createApp();
  const res = await app.request("/", { headers: { "accept-encoding": "identity" } });
  assert.equal(res.status, 200);
  assert.equal(res.headers.get("content-encoding"), null);
});

test("gzipped responses still decode to the same bytes", async () => {
  const app = createApp();
  const plain = await (await createApp().request("/", { headers: { "accept-encoding": "identity" } })).arrayBuffer();
  const res = await app.request("/", { headers: { "accept-encoding": "gzip" } });
...
test("proxy routes that forward upstream bytes are never compressed", async () => {
  // Unauthenticated, so these 401 before reaching upstream -- enough to prove
  // the compression middleware declines the path, which is what issue #76 was
  // about. See UNCOMPRESSED_ROUTES.
  const app = createApp();
...
test("the push stream is never compressed", async () => {
  const app = createApp();
  const res = await app.request("/api/events", { headers: { "accept-encoding": "gzip" } });
  assert.equal(res.headers.get("content-encoding"), null);
});
