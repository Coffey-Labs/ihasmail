...
- **Encryption keys** in Settings › Encryption keys: publish OpenPGP public keys or S/MIME certificates on the account, over Stalwart's `x:PublicKey` registry — list, add, rename, remove. Only public material: no private key is stored, requested or sent. Stalwart parses the key itself and its own rejection is shown verbatim, since it knows exactly what is wrong and ihasmail would only be guessing. Note this release manages keys and nothing more — it does not yet sign, encrypt, decrypt or verify anything with them
...
- Signed and encrypted mail. Key management landed first because it needs nothing secret in the browser. Verifying a signature is the same — it needs only public keys — but signing and decrypting need a private key in a page served by the same host that would handle it, which is a security question to settle before it is a feature to build
